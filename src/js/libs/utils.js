...
utils.animate = ( selector, cls = "bounceIn" ) => {
  ...
  $( document.body ).find( selector ).removeClass( _cls ).addClass( _cls ).one( 'webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', ( e ) => {
...
utils.setupInputMask = () => {
  $( document.body ).find( ".price-input" ).attr( "data-inputmask", "'alias': 'numeric', 'groupSeparator': ',', 'digits': 2,'digitsOptional': false, 'placeholder': '0'" ).change();
  $( document.body ).find( ".qty-input" ).attr( "data-inputmask-regex", "[0-9]{1,4}" ).change();
  $( ":input" ).inputmask( {
